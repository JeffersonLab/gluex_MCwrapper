...
thttp://api.ipapi.com/68.0.33.149?access_key=cc138a088a1a86604716a19dc20ad07a
 */
var IPMapper = {
	map: null,
	...
	latlngbound: null,
	infowindow: null,
	baseUrl: "http://api.ipapi.com/", //"https://freegeoip.net/json/",
	...
	addIPMarker: function(ip){
		...
		if($.trim(ip) != '' && ipRegex.test(ip)){ //validate IP Address format
			var url = encodeURI(IPMapper.baseUrl + ip + "?access_key=cc138a088a1a86604716a19dc20ad07a&callback=?"); //geocoding url
		...
		if($.trim(ip) != '' && ipRegex.test(ip)){ //validate IP Address format
			...
			$.getJSON(url, function(data) { //get Geocoded JSONP data
				if($.trim(data.latitude) != '' && data.latitude != '0' && !isNaN(data.latitude)){ //Geocoding successfull
					...
					var toRet=[]
					toRet.push(latitude)
					toRet.push(longitude)
					totarr.push(latitude)
					totarr.push(longitude)
					totarr.push(1)
