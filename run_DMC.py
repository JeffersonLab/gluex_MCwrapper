...
def main(argv):
        ...
        parser.add_argument('config_file',type=argparse.FileType('r'),help='run_DMC config file')
        parser.add_argument('runrange',help='run number or range of run numbers to produce MC for')
        parser.add_argument('num_events',type=int,help='number of events that are produced')
        parser.add_argument('--variation',default='mc',type=str,help='a valid jana_calib_context variation string (default is "mc")')
        parser.add_argument('--calibtime',default='notime',type=str,help='a valid jana_calib_context calib time string (default is "notime")')
        parser.add_argument('--per_file',default=10000,type=int,help='the number of events you want per file/job (default is 10000)')
        parser.add_argument('--base_file_number',default=0,type=int,help='the starting number of the files/jobs (default is 0)')
        parser.add_argument('--numthreads',type=int,help='sets the number of threads to use to NUMTHREADS.  Note that this will overwrite the NCORES set in MC.config')
        parser.add_argument('--generate',default=1,type=int,choices=[0,1],help='where 0 means that the generation step and any subsequent step will not run (default is 1)')
        parser.add_argument('--geant',default=1,type=int,choices=[0,1],help='where 0 means that the geant step and any subsequent step will not run (default is 1)')
        parser.add_argument('--mcsmear',default=1,type=int,choices=[0,1],help='where 0 means that the mcsmear step and any subsequent step will not run (default is 1)')
        parser.add_argument('--recon',default=1,type=int,choices=[0,1],help='where 0 means that the reconstruction step will not run (default is 1)')
        parser.add_argument('--cleangenerate',default=1,type=int,choices=[0,1],help='where 0 means that the generation step will not be cleaned up after use (default is 1)')
        parser.add_argument('--cleangeant',default=1,type=int,choices=[0,1],help='where 0 means that the geant step will not be cleaned up after use (default is 1)')
        parser.add_argument('--cleanmcsmear',default=1,type=int,choices=[0,1],help='where 0 means that the mcsmear step will not be cleaned up after use (default is 1)')
        parser.add_argument('--cleanrecon',default=0,type=int,choices=[0,1],help='where 0 means that the reconstruction step will not be cleaned up after running (default is 0)')
        parser.add_argument('--batch',default=0,type=int,choices=[0,1,2],help='where 1 means that jobs will be submitted, 2 will do the same as 1 but also run the workflow in the case of swif (default is 0 [interactive])')
        parser.add_argument('--logdir',type=str,help='will direct the .out and .err files to the specified path for qsub')
        parser.add_argument('--projid',help=argparse.SUPPRESS)
        parser.add_argument('--shell',default='csh',help=argparse.SUPPRESS)
        parser.add_argument('--submitter',help=argparse.SUPPRESS)
        ...
        RUNNUM = str(args.runrange).lstrip("0")
        EVTS = int(args.num_events)
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        VERSION=args.variation
        CALIBTIME=str(args.calibtime)
        PERFILE=int(args.per_file)
        BASEFILENUM=int(args.base_file_number)
        GENR=int(args.generate)
        GEANT=int(args.geant)
        SMEAR=int(args.mcsmear)
        RECON=int(args.recon)
        CLEANGENR=int(args.cleangenerate)
        CLEANGEANT=int(args.cleangeant)
        ...
        if BATCHRUN == 0 or BATCHSYS=="NULL":
                print( "Locally simulating "+str(args.num_events)+" "+CHANNEL+" Events")
        else:
                print( "Creating "+WORKFLOW+" to simulate "+str(args.num_events)+" "+CHANNEL+" Events")
        # CREATE WORKFLOW
