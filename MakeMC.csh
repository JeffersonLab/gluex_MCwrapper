...
if ( "$GENR" != "0" ) then #run generation
	...
	if ( "$gen_pre" == "file" ) then
		...
		else if( "$GENERATOR" == "gen_ee_hb" || "$GENERATOR" == "genBH" || "$GENERATOR" == "KLGenerator_hddm_V3") then
	...
	if ( "$GENERATOR" == "genr8" ) then
        ...
        else if ( "$GENERATOR" == "KLGenerator_hddm_V3" ) then
		echo "Configuring KLGenerator_hddm_V3"
		set STANDARD_NAME="KLGenerator_hddm_V3_"$STANDARD_NAME
		echo $STANDARD_NAME
		echo "note: this generator is run completely from command line, thus no config file will be made and/or modified"
	endif
	...
	if ( "$GENERATOR" == "genr8" ) then
		...
		echo "RUNNING KLGenerator_hddm_V3"
		set optionals_line = "kl3"
		echo $PWD
		#echo KLGenerator_hddm_V3 -M$EVT_TO_GEN -F$STANDARD_NAME.root -L/home/sfegan/KLF/KLGenerator_hddm_V3 -Ekaon:histo:$GEN_MIN_ENERGY:$GEN_MAX_ENERGY -R$optionals_line
		echo KLGenerator_hddm_V3 -M1000 -F$STANDARD_NAME.root -Ekaon:histo:1.0:4.0 -Rkl3
		set generator_return_code=$status
	endif
