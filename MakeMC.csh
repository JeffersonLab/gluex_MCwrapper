...
if ( "$GENR" != "0" ) then #run generation
	...
	if ( "$GENERATOR" == "genr8" ) then
		...
		cp $CONFIG_FILE ./$STANDARD_NAME.conf
	else if ( "$GENERATOR" == "gen_tcs_bh" ) then
		echo "configuring gen_tcs_bh"
		set STANDARD_NAME="gen_tcs_bh_"$STANDARD_NAME

		#set MCWRAPPER_CENTRAL_CNTR=`$runGen printenv MCWRAPPER_CENTRAL | tail -n1`
		set MCWRAPPER_CENTRAL_CNTR="/w/halld-scshelf2101/home/gchung/tcs_sim2/gluex_MCwrapper" # for testing purpose
	...
	if ( "$GENERATOR" == "genr8" ) then
		...
		set RANDOMnum=`bash -c 'echo $RANDOM'`
		echo "Random number used: "$RANDOMnum
		echo "Using MakeMC.csh"
		# Read config file
		foreach line ("`grep ':' $CONFIG_FILE | grep -v '^[[:space:]]*#'`")
		    set key=`echo $line | cut -d':' -f1 | tr -d ' '`
		    set val=`echo $line | cut -d':' -f2- | sed 's/^[ \t]*//; s/[ \t]*$//'`
